...
describe('filebundle', () => {
   ...
   it('uses an AppKit file URL on the macOS pasteboard without interpolating the path', async () => {
      ...
      assert.match(script, /pasteboard\.writeObjects\(\[ObjC\.unwrap\(fileURL\)\]\)/)
      assert.match(script, /pasteboardItems\.count/)
      assert.match(script, /NSPasteboardTypeFileURL/)
      assert.match(script, /stringForType/)
      assert.match(script, /itemCount\s*<\s*1/)
      assert.doesNotMatch(script, /fileURL\.js/)
