...
export const createProgressSection = (
  ...
  const lines = [`Progress: ${createProgressBar(completed, total)} ${progressPercentage(completed, total)}%`];
  if (current) {
    const label = state === 'Failed' ? 'Failed at' : state === 'Interrupted' ? 'Interrupted at' : 'Current';
...
export class CommitProgressReporter implements CommitProgressCallbacks {
  ...
  private render(forceFallback: boolean): void {
    if (this.interactive) {
      ...
      if (lineCount > lines.length) readline.moveCursor(process.stdout, 0, lines.length - lineCount);
